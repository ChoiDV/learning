...
public class CustomerDao {
	...
	public static final int CUSTOMER_EXISTENT = 0; // 있는 아이디 아이디 중복체크할때
	public static final int CUSTOMER_NONEXISTENT = 1; // 없는 아이디 아이디 중복체크할때
	...
	public int confirmCid(String cid) {
		...
		try {
			...
			if (rs.next()) {
				...
				System.out.println("회원가입 가능한 ID");
			}
	...
	public int insertCustomer(CustomerDto dto) {
		...
		try {
			...
			//System.out.println(result == SUCCESS ? "회원가입 성공" : "회원가입 실패"); // 여기서는 회원가입 실패가 뜰수없음 실패하면 exception으로 떨어짐
	...
	public int updateCustomer(CustomerDto dto) {
		...
		try {
			...
			System.out.println(e.getMessage()+"회원정보 수정 실패 : " + dto);
		} finally {
			try {				
				if (pstmt != null) {
					pstmt.close();
	...
	// 회원 리스트 보기( 첫화면 main.jsp에서 사용할 부분 - top-N 구문 사용)
	public ArrayList<CustomerDto> listCustomer(int startRow, int endRow){
		...
		try {
			...
			while(rs.next()) {
				...
				String cemail = rs.getString("cemail");
				String caddress = rs.getString("caddress");
				customer.add(new CustomerDto(cid, cpw, cname, null, cemail, caddress, null, null, null));
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		}  finally {
