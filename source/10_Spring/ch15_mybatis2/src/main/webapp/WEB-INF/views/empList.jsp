...
<head>
  ...
  <body>
	<c:set var="num" value="${paging.totCnt - paging.startRow + 1 }"/> <!--  순서번호 역번호 ex) 14, 13, 12, 11 ... 출력되게-->
	<table>
		...
		<c:forEach items="${empList }" var="emp">
			<tr onclick="trclicked(${emp.empno})">
				...
				<td><fmt:formatNumber value="${emp.sal }" pattern="#,###"/></td><!--  groupingUsed="true"  이렇게 넣어도됌-->
			</tr>
	...
	<c:if test="${paging.startPage > paging.blockSize }">
		[ <a href="${conPath }/empList.do?pageNum=${paging.startPage-1 }">이전으로</a>]
	</c:if>
	...
	<c:if test="${paging.endPage < paging.pageCnt }">
		[ <a href="${conPath }/empList.do?pageNum=${paging.endPage+1 }">다음으로</a>]
	</c:if>	
  </body>
</html>