...
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM BOOK ORDER BY BTITLE) A)
    WHERE RN BETWEEN 1 AND 3;
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM BOOK WHERE BTITLE LIKE '%'||'JSP'||'%' ORDER BY BTITLE) A)
    WHERE RN BETWEEN 1 AND 3;
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM BOOK WHERE BWRITER LIKE '%'||'작'||'%' ORDER BY BTITLE) A)
    WHERE RN BETWEEN 1 AND 3;
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM BOOK WHERE BTITLE LIKE '%'||'JSP'||'%' OR BWRITER LIKE '%'||'작'||'%' ORDER BY BTITLE) A)
    WHERE RN BETWEEN 1 AND 3;
