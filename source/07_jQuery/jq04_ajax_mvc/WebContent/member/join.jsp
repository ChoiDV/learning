...
<script>
	$(document).ready(function() {
		...
		$('input[name="mPwChk"]').keyup(function() { 
			...
			if (mPw == mPwChk) {
				$('#pwChkResult').html('<img class="result" src="../img/checkimg.jpg" alt="1">');
			} else {
				$('#pwChkResult').html('No');
			}
		}); // mPwChk.keyup 이벤트
		$('form').submit(function() {
			...
			//	var pwChkResult = $('#pwChkResult').text().trim();  text로 성공 메시지 작성했을경우
			var pwChkResult = $('.result').attr('alt');  // 나는 이미지
			if (idConfirmResult != "사용가능한 ID 입니다" ) {
				alert('사용가능한 ID로 가입하세요'); // 중복체크 다시해라!!!
				$('#idConfirmResult').focus();
				return false;
			} else if (pwChkResult != 1) {
				alert('비밀번호를 확인하세요');
				return false;
			}
		}); // form 태그 이벤트
		...
		<table>
			<tr>
				...
				<td><input type="text" name="mId" required="required">
					<!-- 버튼 넣을때는 이거 사용 <input type="button" value="중복확인"> --></td>
			</tr>
			<tr>
				<td></td>
				<td>
			...
			<tr>
				<th>비밀번호 확인</th>
				<td><input type="password" name="mPwChk" required="required"><span
					id="pwChkResult"> &nbsp; </span></td>
			</tr>
			<!--   			<tr> -->
			<!--   				<td></td> -->
			<!--   				<td> -->
