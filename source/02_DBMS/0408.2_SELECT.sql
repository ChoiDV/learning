...
SELECT EMPNO AS 사번, ENAME AS 이름, SAL AS 급여, JOB FROM EMP;  -- " " 를 빼도되지만 스페이스를 쓸때는 꼭 " " 사용
SELECT EMPNO 사번, ENAME 이름, SAL 급여, JOB FROM EMP;
SELECT EMPNO "사 번", ENAME "이 름", SAL "급 여", JOB FROM EMP;
SELECT EMPNO NO, ENAME NAME, SAL SALARY FROM EMP; -- TITLE : NO, NAME, SALARY  -- 자바에서 받아올때는 TITLE : NO, NAME, SALARY
...
-- 날짜 표기법 셋팅 (현재 : YY/MM/DD OR  RR/MM/DD) 로 현재 셋팅 한글 윈도우라서 한글셋팅
ALTER SESSION SET NLS_DATE_FORMAT = 'MM-DD-RR';   -- RR 은 YY랑 똑같은거
...
SELECT ENAMEK, SAL "현재급여", SAL+100 "올릴급여" FROM EMP WHERE DEPTNO=10;
    ...
    -- 산술연산의 결과는 NULL포함하면 결과도 NULL 하나라도 NULL인경우 결과값이 NULL 이 됌
    -- NVL(NULL일 수도 있는 필드명, 대치값) 반드시 매개변수 둘은 타입 같아야 함
SELECT ENAME, SAL, COMM, SAL*12+COMM FROM EMP;
SELECT ENAME, SAL, COMM, SAL*12+NVL(COMM, 0) FROM EMP;
    -- 모든 사원의 사원명(ENAME), 상사의 사번(MGR)을 출력(상사없으면 0) 상사의 사번이 숫자기때문에 없음이 안되고 0으로 해야함
...
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL<2000;
 ...
 -- # 날짜 설정 (YYYY 년도 4자리, RR 년도2자리, M월, D일)
...
SELECT EMPNO, ENAME, JOB, HIREDATE FROM EMP
        WHERE TO_CHAR(HIREDATE, 'RR/MM/DD' )  >= '81/02/01';  -- 단일행 함수  (4장 참조)
        
 SELECT EMPNO, ENAME, JOB, HIREDATE FROM EMP
        WHERE HIREDATE >=  TO_DATE('81/02/01', 'RR/MM/DD');     --단일행 함수 (4장 참조)
...
-- 8. SQL 연산자 (BETWEEN, IN ,LIKE, IS NULL )
    --(1)  BETWEEN
    -- ex. SAL이 1500 이상 3000 이하인 사번 이름 급여
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL>=1500 AND SAL <= 3000;

SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL BETWEEN 1500 AND 3000;
                                                                                     -- BETWEEN A AND B; 인데 반드시 A에 작은값 B에 큰값;   , A 와 B 포함
SELECT * FROM EMP WHERE NOT SAL BETWEEN 1500 AND 3000;
                                                                                    --  ex. 1500미만, 3000 초과하는 모든 필드
...
SELECT * FROM EMP WHERE ENAME >= 'B' AND ENAME <= 'D' AND ENAME<>'D';

        -- (2) IN :  데이터 IN (A, B, C ...)
        -- ex. 부서 번호가 10,20,40번인 부서 사원의 모든 필드
SELECT * FROM EMP WHERE DEPTNO=10 OR DEPTNO=20 OR DEPTNO=40;
SELECT * FROM EMP WHERE DEPTNO IN ( 10,20,40);   -- IN 연산자

        -- ex. 부서 번호가 10,20,40번 부서를 제외한 사원의 모든 필드    NOT IN 연산자
SELECT * FROM EMP WHERE DEPTNO != 10 AND DEPTNO<>20 AND DEPTNO ^= 40;
SELECT * FROM EMP WHERE DEPTNO NOT IN (10,20,40);

        --ex. 사번이 7902, 7788, 7566 인 사원의 모든 필드
SELECT * FROM EMP WHERE EMPNO IN (7902, 7788, 7566);

--------------------------------------------LIKE
        -- (3) LIKE 연산 :  문법 =  데이터 LIKE 패턴(%: 0 글자 이상, _:한글자)
        --  LIKE 패턴 와일드카드  % 와 _  %는 0글자 이상 _는 한글자 이상
...
SELECT * FROM EMP WHERE ENAME LIKE '%S%';
SELECT * FROM EMP WHERE ENAME LIKE '%%%';   -- 모든 필드를 의미
...
-- ----------------------9. 정렬 ( 오름차순, 내림차순)   문자는 a가 작은것 날짜는 옛날이 작은것
SELECT ENAME, SAL FROM EMP ORDER BY SAL; -- 급여 오름차순 정렬
SELECT ENAME, SAL FROM EMP ORDER BY SAL ASC;  -- 급여 오름차순 정렬 ASC 생략 가능

SELECT ENAME, SAL FROM EMP ORDER BY SAL DESC;  -- 급여 내림차순 정렬

SELECT ENAME, SAL FROM EMP ORDER BY ENAME;  -- 이름 ABCD 순으로 정렬 이름 오름차순
SELECT ENAME, SAL FROM EMP ORDER BY ENAME DESC;   -- 이름 내림차순

SELECT ENAME, HIREDATE FROM EMP ORDER BY HIREDATE;   -- 날짜 순서대로 정렬  날짜 오름차순
SELECT ENAME, HIREDATE FROM EMP ORDER BY HIREDATE DESC;   -- 날짜 내림차순. 최신 순서대로 정렬

        -- 이름, 연봉(SAL*12+COMM)을 출력(연봉이 높은 순으로), 연봉이 같은 경우 이름순으로
SELECT ENAME, SAL*12+NVL(COMM,0) AS ANNUAL_SAL FROM EMP  ORDER BY SAL*12+NVL(COMM,0) DESC, ENAME;
        -- ORDER BY 절에는 별칭 사용 가능 ( FROM -> WHERE -> SELECT -> ORDER )
SELECT ENAME, SAL * 12 + NVL(COMM , 0 ) AS "ANNUAL_SAL" FROM EMP ORDER BY ANNUAL_SAL DESC, ENAME;
...
SELECT ENAME, JOB, HIREDATE FROM EMP WHERE HIREDATE BETWEEN TO_DATE('1981/02/20','YYYY/MM/DD') AND TO_DATE('1981/05/01' , 'YYYY/MM/DD');
