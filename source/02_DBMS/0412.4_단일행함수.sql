...
SELECT ENAME, INITCAP(ENAME) FROM EMP;  -- 단일행 함수
    ...
    -- 그룹 함수  :  (INPUT N행 받아서 -> OUTPUT 1행)  -- 5장
SELECT SUM(SAL) FROM EMP;  -- 전체 SAL 합
SELECT DEPTNO, SUM(SAL) FROM EMP GROUP BY DEPTNO; -- 부서별 SAL 합

    --  ★ 단일행 함수 종류 : 숫자관련함수, 문자관련함수, 날짜관련함수(예약어) , 형변환 함수, NVL() , ETC, ...★

-- 1. 숫자관련 함수
        --  DUAL 테이블 : 오라클에서 제공하는 1행1열짜리 dummpy table
DESC DUAL;
SELECT * FROM DUAL;
SELECT * FROM DUMMY;
DESC DUMMY;
...
SELECT CEIL(34.5678*100)/100 FROM DUAL;  -- 소수점 둘째자리에서 올림

SELECT ROUND(34.5678) FROM DUAL;  -- 소수점에서 반올림
SELECT ROUND(34.5678,2) FROM DUAL; -- 소수점 둘째자리에서 반올림
SELECT ROUND(34.5678,-1) FROM DUAL; -- 일의 자리에서 반올림

SELECT MOD(9,2) FROM DUAL;  -- 나머지 연산자  9/2 의 나머지 출력
...
SELECT INITCAP ('WELCOME TO ORACLE') FROM DUAL;
        -- ex. EMP 테이블에서 JOB이 'manager'인 직원의 모든 필드를 출력해라
SELECT * FROM EMP
...
SELECT LENGTHB ('오라클') FROM DUAL; -- 바이트수 : 9
 ...
 --  INSTR(STR, 찾을글자, 시작위치) : STR에서 시작위치부터 찾아서 찾은 글자의 위치 (없으면 0 )
SELECT INSTR ('ABCABC','B') FROM DUAL;  -- 처음부터 찾아서 처음 나오는 B의 위치 : 2
SELECT INSTR('ABCABC','B',3) FROM DUAL;  -- 앞 3번째부터 찾아서 처음 나오는 B의 위치 : 5
SELECT INSTR('ABCABC','B',-3) FROM DUAL; -- 뒤  3번째부터 뒤에서 앞으로 찾아 처음 나오는 B의 위치 : 2
...
SELECT * FROM EMP WHERE INSTR(HIREDATE, '09',7) =0;
        ...
        -- (6) LPAD (STR, 자리수, 채울문자) STR을 자리수만큼 확보하고 왼쪽 빈자리에 채울문자 출력
        --  RPAD (STR, 자리수, 채울문자) STR을 자리수만큼 확보하고 오른쪽 빈자리에 채울문자 출력
 SELECT LPAD('ORACLE',10,'#') FROM DUAL;  --  = ####ORACLE
 SELECT RPAD('ORACLE',10,'#') FROM DUAL; -- = ORACLE####
 SELECT ENAME, SAL, LPAD(SAL, 6 , '*') FROM EMP;
 
 
        -- ex. 사번, 이름 (7788 S****, 7864 S****, 이름은 앞에 한글자만 출력하고 나머지는 *)
...
SELECT EMPNO, LPAD(LPAD(SUBSTR(ENAME,-1,1),LENGTH(ENAME),'*'), 10,'  ') "이름", RPAD(TO_CHAR(HIREDATE, 'RR/MM/'),8,'*') "HIREDATE" FROM EMP;
-- TO_CHAR(HIREDATE, 'RR/MM')||'**' "HIREDATE' FROM EMP; 도 가능 날짜
        -- ex. 이름의 세번째 자리가 R인 사원의 모든 필드 (LIKE 사용, INSTR 사용, SUBSTR 사용)
SELECT * FROM EMP
    WHERE ENAME LIKE '__R%';
    
SELECT * FROM EMP
    WHERE INSTR(ENAME,'R',3) = 3;  --   만약에 RARA 라는 사람이 있으면 3번째부터 찾지않으면  안나옴 
...
SELECT ENAME, HIREDATE, FLOOR(SYSDATE-HIREDATE) "근무일수" FROM EMP;
        -- 근무 주 수
SELECT ENAME, HIREDATE, FLOOR((SYSDATE-HIREDATE)/7) "근무 주 수" FROM EMP;
        -- ex. 이름, 입사일, 근무 년수
SELECT ENAME, HIREDATE, FLOOR((SYSDATE-HIREDATE)/365) "근무 년 수" FROM EMP;
        -- ex. 이름 , 입사일, 근무 개월 수

        -- (3) MONTHS_BETWEEN(날짜, 날짜) : 두 날짜간의 개월수 계산
        
        -- ex. 이름, 입사일, 근무 개월 수
SELECT ENAME, HIREDATE, FLOOR(MONTHS_BETWEEN(SYSDATE,HIREDATE)) "근무 개월 수" FROM EMP;
        -- ex. 이름, 입사한 이후 받은 급여와 상여(급여는 매달 sal을 받고, comm은 1년에 2회 받음)
SELECT ENAME, SAL*TRUNC(MONTHS_BETWEEN(SYSDATE,HIREDATE)) "총 급여",
            NVL(COMM,0)* TRUNC((SYSDATE-HIREDATE)/365)*2 "총 상여금" 
                FROM EMP;
                

        -- (4) ADD_MONTHS(날짜, 개월 수 ) : 특정시점부터 몇개월 후의 날짜
...
SELECT ROUND(SYSDATE, 'YEAR') FROM DUAL;  -- 결과 : 가까운 1월1일로 반올림
SELECT ROUND(SYSDATE, 'MONTH') FROM DUAL; -- 결과 : 가까운 1일로 반올림 (1~15일은 이번달 1일 , 16 이후는 다음달 1일)
SELECT ROUND(SYSDATE, 'DAY') FROM DUAL;    -- 결과 : 가까운 일요일을 반환
SELECT ROUND(SYSDATE) FROM DUAL;   -- 결과 : 가까운 0시
SELECT TO_CHAR(ROUND(SYSDATE), 'RR/MM/DD HH24:MI') FROM DUAL;

SELECT TRUNC(SYSDATE, 'YEAR') FROM DUAL;  -- 결과 : 올해 1일
SELECT TRUNC(SYSDATE, 'MONTH') FROM DUAL; -- 결과 : 이번달 1일
SELECT TRUNC(SYSDATE, 'DAY') FROM DUAL;    -- 결과 :  지난주 일요일
...
--  YYYY (년도 4자리) YY,RR (년도 2자리) ,  MM (월) , DD (일), DAY (요일) ,  DY(요일 요약표현) 
--  AM 이나 PM (오전/오후),  HH12 (12시간 단위) , HH24 (24시간) MI (분) , SS(초)
...
--  . : 소수점
--  $ : 통화단위 $가 앞에 붙을 수 있음
--  L : 지역 통화 단위가 앞에 붙을 수 있음
SELECT TO_CHAR(123456, '9,999,999.99') FROM DUAL;    -- 9가 자릿수를 뜻함
...
SELECT TO_DATE('2022-0412' , 'YYYY-MMDD') FROM DUAL;
    -- exx. 81/5/1 ~83/5/1 사이에 입사한 직원의 모든 필드( TO_DATA 사용)
SELECT * FROM EMP
    WHERE HIREDATE BETWEEN TO_DATE('810501','RRMMDD') AND TO_DATE('83/05/01' , 'RR/MM/DD');

-- (4) TO_NUMBER (문자, '패턴')
SELECT '123'+1 FROM DUAL;
SELECT '1,234' +1 FROM DUAL;  -- 이거는 불가능함 그래서 
SELECT TO_NUMBER('1,234', '9,999')+1 FROM DUAL;

-- (5) NVL( NULL 일수도 있는 데이터, NULL 이면 대신할 값) - 매개변수 2개는 타입 일치해야함
SELECT ENAME, NVL(COMM, 0 ) FROM EMP;
        -- ex. 이름, 상사의 사번(mgr)을 출력하되, 상사의 사번이 NULL 일 경우 'CEO' 출력
SELECT ENAME, NVL(TO_CHAR(MGR), 'CEO') FROM EMP;

-- 6. ETC
-- (1) EXTRACT : 년도나 월이나 날짜만 숫자로 추출하고자 할때
...
SELECT ENAME, HIREDATE, NEXT_DAY( ADD_MONTHS(HIREDATE, 6),'월') "월요일" FROM EMP;
--4. 이름, 입사일, 입사일로부터 현재까지의 개월수, 급여, 입사일부터 현재까지의 받은 급여의 총계를 출력
SELECT ENAME, HIREDATE, 
    FLOOR(MONTHS_BETWEEN(SYSDATE,HIREDATE)) "총 개월 수",  
        SAL, FLOOR(MONTHS_BETWEEN(SYSDATE,HIREDATE))*SAL "총 급여"
            FROM EMP;
...
--7521		       WARD		******1250
--……. 
SELECT EMPNO, LPAD(ENAME,10,' ') "ENAME" , LPAD(SAL,10,'*') "급여" FROM EMP;

-- 12. 다음과 같은 결과를 검색할 수 있는 SQL 문장을 작성하시오
-- EMPNO	 ENAME 	입사일
-- 7369	  SMITH		1980-12-17
-- ….
SELECT EMPNO, RPAD(ENAME,10,' ') "ENAME" , TO_CHAR(HIREDATE, 'YYYY-MM-DD') "입사일" FROM EMP;
