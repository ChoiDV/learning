...
SELECT ENAME, ROUND(SAL, -2) FROM EMP;  -- 단일행 함수 (4장)
SELECT MAX(SAL) FROM EMP; -- SAL 중에 가장큰 SAL 출력  -- 그룹함수 (5장)
SELECT DEPTNO, MAX(SAL) FROM EMP GROUP BY DEPTNO;  -- 그룹함수 (5장)
SELECT ENAME, MAX(SAL) FROM EMP; -- 최대 급여를 받는 사람의 이름 (6장) - 에러(단일행과 다중행을 같이 써서) (6장. 서브쿼리)

-- ★ 1.그룹함수들 실습
SELECT ROUND(AVG(SAL),2) FROM EMP;
SELECT COUNT(*) FROM EMP;   -- EMP 테이블의 행(ROW) 개수
SELECT AVG(SAL), SUM(SAL)/COUNT(SAL) FROM EMP;

-- 모든 그룹 함수는 NULL 값을 제외
...
-- 그룹함수 MIN, MAX, COUNT 함수는 숫자형, 문자형, 날짜형 모두 사용가능
-- MIN(ENAME)은 아스키코드가 가장 작은애 : A
...
SELECT MAX(HIREDATE)||'최근입사일: '||TO_CHAR(SYSDATE-MAX(HIREDATE),'99,999')||'일 근무,'||
    MIN(HIREDATE)||'최초 입사일: '||TO_CHAR(SYSDATE-MIN(HIREDATE),'99,999')||'일 근무'    
    FROM EMP;

--2번째 방법                                      --   현재날짜 - 입사일 한것중에 MIN값을 TO_CHAR(값,'99,999')
SELECT MAX(HIREDATE) || '최근 입사일 : ' || TO_CHAR(MIN(SYSDATE-HIREDATE),'99,999') || '일 근무, ' ||
    MIN(HIREDATE) || '최초입사일 : ' || TO_CHAR(MAX(SYSDATE-HIREDATE),'99,999') || '일 근무'
        FROM EMP;
...
SELECT DEPTNO, MAX(SAL), MIN(SAL), AVG(SAL), SUM(SAL) FROM EMP GROUP BY DEPTNO;   -- GROUP BY 절에는 컬럼의 별칭 사용 불가
...
SELECT DEPTNO, COUNT(*), MAX(SAL), MIN(SAL), AVG(SAL) -- SELECT 절에서는 필드 별칭 사용 가능 
    FROM EMP
        WHERE SAL<5000   -- WHERE 절에서는 필드 별칭 사용 불가
            GROUP BY DEPTNO  -- GROUP BY 절에서는 필드 별칭 사용 불가 
                ORDER BY DEPTNO;  -- ORDER BY 절에서는 필드 별칭 사용 가능 
    --ex. 부서명별 사원수, 최대급여, 최소급여, 평균급여(소수점 2자리)
SELECT DNAME, COUNT(*), MAX(SAL), MIN(SAL), ROUND(AVG(SAL),2)  -- (사원수가 많은 순으로 정렬)
    FROM EMP E, DEPT D
...
-- 부서번호 별 평균 급여( 평균급여가 2000 이상인 부서만 출력, 평균 급여 순으로 정렬)
SELECT DEPTNO, AVG(SAL)
    FROM EMP
        GROUP BY DEPTNO
            HAVING AVG(SAL) >= 2000  -- WHERE절인데 그룹함수 결과의 조건을 적는곳   -- HAVING 절에는 별칭 사용 불가
                ORDER BY AVG(SAL);


-- SAL이 5000 미만인 사원에 대해 부서명별 최소급여와 최대 급여, 평균급여 (단, 평균 급여가 1800이상인 부서명에 대해서
    -- 평균급여순으로 정렬해서 출력하시오 .
...
SELECT DEPTNO, NVL(JOB,'소계'), SUM(SAL) FROM EMP
